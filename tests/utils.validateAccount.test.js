...
describe("GET /utils/validate-account", () => {
  it("returns isValid: true with reason: null for a valid public key", async () => {
    const res = await request(app).get(`/utils/validate-account?id=${VALID_KEY}`);
  ...
  it("returns isValid: false with a reason when the key has a wrong prefix", async () => {
    ...
    const wrongPrefix = "SAAZI4TCR3TY5OJHCTJC2A4QSY6CJWJH5IAJTGKIN2ER7LBNVKOCCWN";
    const res = await request(app).get(`/utils/validate-account?id=${wrongPrefix}`);
    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
  ...
  it("returns isValid: false with a reason when the key is too short", async () => {
    ...
    const res = await request(app).get(`/utils/validate-account?id=${tooShort}`);
    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
  ...
  it("returns isValid: false with a reason when the key contains invalid characters", async () => {
    ...
    const invalidChars = "G0AZI4TCR3TY5OJHCTJC2A4QSY6CJWJH5IAJTGKIN2ER7LBNVKOCCWN";
    const res = await request(app).get(`/utils/validate-account?id=${invalidChars}`);
    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
